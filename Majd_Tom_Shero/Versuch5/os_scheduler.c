...
ISR(TIMER2_COMPA_vect) {
	...
	if ((*os_getProcessSlot(os_getCurrentProc())).state == OS_PS_RUNNING){
	...
	switch(os_getSchedulingStrategy()){
		...
		if (schedulingInfo.timeSlice != 0)	schedulingInfo.timeSlice--;
	...
	// set blocked processes to ready
	for (int rare = 1; rare < MAX_NUMBER_OF_PROCESSES; rare++){
		if (os_processes[rare].state == OS_PS_BLOCKED) 
			os_processes[rare].state = OS_PS_READY;
	}
...
ProcessID os_exec(ProgramID programID, Priority priority) {
   //we don't want an interrupt here, so enter critical section
	...
	// reset the scheduling information for executed process
...
/*!
 * a wrapper function for all processes 
 * function address given in os_exec() to prepare process stack.
 * allows preemptive scheduling.
 */
void os_dispatcher(void){
	Program* j = os_lookupProgramFunction(os_getProcessSlot(currentProc)->progID);
	int tmp_count = criticalSectionCount;
	(*j)();
	os_kill(currentProc);
	for (int i = 0; i < criticalSectionCount-tmp_count; i++)
	{
		os_leaveCriticalSection();
	}
	os_yield();
 ...
 * resets the values of the process
 *
...
void os_startScheduler(void) {
   ...
   currentProc = 0;
   //set its state to running assign its stack-pointer to SP
   (*os_getProcessSlot(currentProc)).state = OS_PS_RUNNING;
...
void os_yield(void) {
	...
	int tempCriticalSectionCount = criticalSectionCount;
	...
	// Make sure timer-interrupts are still applicable 
	TIMSK2 |= 0b00000010;
	...
	cli(); 
	SREG |= gieb;
	criticalSectionCount = tempCriticalSectionCount;
