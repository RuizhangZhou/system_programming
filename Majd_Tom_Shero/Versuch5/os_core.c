...
void os_errorPStr(char const* str) {
	...
	uint8_t sreg = SREG &= 0b10000000; 
    // deactivate 7th Bit of SREG to deactivate interrupts
	SREG &= (0b01111111);
	
	//clear LCD and write the ERROR
	lcd_clear();
	lcd_writeString(str);
    ...
    //wait untill ENTER+ESC is pressed
	while( os_getInput() != 0b00001001 ){ /* Wait */ }

    //once pressed clear the ERROR message
	lcd_clear();
	
    //apply old SREG state
	SREG |= sreg;
}
