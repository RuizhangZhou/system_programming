...
typedef enum {
  ...
  OS_PR_FRONTPAGE,    //!< Request to show the front page showing how many
                      //!< processes. are running and which one is currently
                      //!< active.
  ...
  OS_PR_PRIORITY_SELECT, //!< Request to show the page in which a process can be
                         //!< selected whose priority should be changed.
  OS_PR_PRIORITY_SHOW,   //!< Request to show the page on which the new priority
                         //!< of the selected process can be chosen.
  OS_PR_PRIORITY, //!< Request to set the priority of the selected process to
                  //!< the chosen value.
  OS_PR_SCHEDULING_SELECT, //!< Request to show the scheduling strategy
                           //!< selection.
  OS_PR_SCHEDULING, //!< Request to set the scheduling strategy to the selected.
  OS_PR_ALLOCATION_SELECT, //!< Request to show the allocation strategy
                           //!< selection for the previously selected heap.
  OS_PR_ALLOCATION, //!< Request to set the allocation strategy of the selected
                    //!< heap to the newly chosen.
  OS_PR_SHOW_HEAP, //!< Request to open the heap sub menu for the selected heap.
  OS_PR_ERASE_HEAP //!< Request to completely erase the contents (map and use)
                   //!< of the selected heap.
} PermissionRequest;
...
AccessPermission os_askPermission(PermissionRequest ap, RequestArgument ra,
                                  RequestArgumentFlag raf, char const **reason);
