...
How does it work?
  At its core, the TM is basically a generic engine that controls a dynamic
graph (!) of pages. Usually the dynamic graph of pages degenerates to a static
tree, rooted at the "root-page".

Control flow;
  When the TM is invoked, by calling `os_taskManMain`, it will automatically
push the root-page to its display-buffer. The user may then select pages which
are pushed on top of this root-page. When the buffer is empty, the TM terminates
and returns control to its caller. All the work is done in `os_taskManMain`
while the pages are specified as functions that can peek on the call stack to
determine their respective context. Pages **NEVER** block (i.e. no busy
waiting). All user interaction is done in `os_taskManMain`. If you wish to
understands the details, there is no way around understanding how
`os_taskManMain` works.

Page specification;
  To allow the single pages to be minimalistic, and quite simple to extend, a
lot of helper functions and macros are implemented which can be used in order to
 ...
 *  This is hard coded, because we have to allocate stack space before calling
 * the pages. If this value is too small, you will not be able to descend beyond
 * the given level.
 */
 ...
 *  It is never used directly but utilizes a macro to use a stack variable as
 * inputBuffer. \param inputBuffer The byte where to store the state of the
 * buttons. \returns The content of the buffer.
 */
 ...
 *  generally in common, that they have an index which is currently displayed
 * and a range where this index can move. Note that indices may be invalid.
 */
 ...
 *  the previous page.
 *  However, due to this mechanism, the native call-stack only holds one
 * function call, but is oblivious to the parameters of parent pages. Therefore
 * we have to construct or own call-stack, stored in a variable of this type.
 */
struct ParamStack {
  /*!
...
bool os_taskManOpen() { return tm_open; }
...
#define READ_BTN(BTN) ((buttonInput >> (BTN)) & 1)
   ...
   */
  ParamStack stack = {.top = TM_NESTING_DEPTH, .size = TM_NESTING_DEPTH};
  stack.top--;
  ...
  while (stack.top < stack.size) {
    ...
    do {
      ...
      if (pageResult.success) {
        /*
         * Note how the user is not even bothered in case the current page
         * failed. After polling here, the poll-result that was read here, will
         * be used in the remainder of this iteration. So even if the user
         * manages to release a button faster than we can process it, we will
         * still know it was pressed (updateInput() has heavy side effects, as
         * it is a macro).
         */
        while (!updateInput())
          ;
      ...
      if (READ_BTN(ES) || !pageResult.success) {
         ...
         */
        stack.top++;
      } else if (READ_BTN(OK) && (pageResult.child.call) &&
                 stack.top) { // Only descend if the nesting level has not been
                              // reached!
        /*
      ...
      } else if ((READ_BTN(DN) || READ_BTN(UP)) &&
                 (stack.pages[stack.top].range > 1)) {
        ...
        direction +=
            READ_BTN(DN) + (READ_BTN(UP) * (stack.pages[stack.top].range - 1));
      } else {
        /*
    ...
    // This can occur if our design-time estimate of the stack size was too
    // small. { stack.top + 1 != 0 }
  }
  tm_open = false;
  lcd_clear();
 ...
 *  -> The members of SchedulingStrategy may not be strictly increasing, so
 * there may be gaps in the interval [0,SCHEDULING_STRATEGY_COUNT] and more
 * importantly, some strategies might have a value >=SCHEDULING_STRATEGY_COUNT.
 *     Gaps are no problem for the TM engine.
 */
#if VERSUCH >= 5
#define SS_MAX_COUNT                                                           \
  (MAX6(OS_SS_RUN_TO_COMPLETION, OS_SS_RANDOM, OS_SS_EVEN, OS_SS_ROUND_ROBIN,  \
        OS_SS_INACTIVE_AGING, OS_SS_MULTI_LEVEL_FEEDBACK_QUEUE) +              \
   1)
#else
#define SS_MAX_COUNT                                                           \
  (MAX5(OS_SS_RUN_TO_COMPLETION, OS_SS_RANDOM, OS_SS_EVEN, OS_SS_ROUND_ROBIN,  \
...
#if TM_COMPILE_HEAP_SUPPORT
#define MS_MAX_COUNT                                                           \
  (MAX4(OS_MEM_FIRST, OS_MEM_NEXT, OS_MEM_BEST, OS_MEM_WORST) + 1)
#endif
...
#define make_pagehandler(NAME, CHILD, CHILDARG, CHILDRANGE, REQ, REQFLAG, ARG) \
  ...
  static TM_PAGE(NAME) {                                                       \
    ...
    pageHandlerWrapper(p, result, NAME##_user,                                 \
                       ((CHILD) == tm_null) ? 0 : (CHILD), (CHILDARG),         \
                       (CHILDRANGE), REQ, ra, OS_RAF_##REQFLAG);               \
  }                                                                            \
 ...
 *  \param result The information that the page is supposed to return to the
 * os_taskManMain function. \param pageHnd The custom user function, actually
 * implementing the page's functionality. \param childHnd Which child page
 * should be returned by default? This can still be overridden in the custom
 * function. \param childArg What would be the initial argument to our child
 * page? \param childRange What parameter range does our child page have? \param
 * pr The permission request specifier for this page's feature. This has to do
 * with user privileges. \param ra The request argument union. \param raf A
 * flag, indicating which union member to extract from ra.
 */
static void pageHandlerWrapper(ParamStack const *p, PageResult *result,
                               bool (*pageHnd)(ParamStack const *, PageState *),
                               tm_page *childHnd, uint16_t childArg,
                               uint16_t childRange, PermissionRequest pr,
 ...
 */
make_pagehandler(tm_startProg, tm_startProg_exec, 0, 1, OS_PR_START_PROG_SELECT,
                 null, 0) {
 ...
 */
make_pagehandler(tm_startProg_exec, tm_null, 0, 0, OS_PR_START_PROG, prog,
                 peekStack(1).param) {
...
// procMutator and procMutatorConfirm is only compiled if it is used, thus a
// warning is avoided
 ...
 */
static bool procMutator(ParamStack const *p, char const *note,
                        uint32_t stateCondition) {
  uint16_t const page = peekStack(0).param;
 ...
 *  \returns true.
 */
static bool procMutatorConfirm(ParamStack const *p, char const *note1,
                               char const *noteIdle,
                               bool (*action)(ProcessID)) {
 ...
 */
make_pagehandler(tm_killProc, tm_killProc_kill, 0, 1, OS_PR_KILL_SELECT, pid,
                 peekStack(0).param) {
 ...
 */
make_pagehandler(tm_killProc_kill, tm_null, 0, 0, OS_PR_KILL, pid,
                 peekStack(1).param) {
  return procMutatorConfirm(p, PSTR("Killing"), PSTR("Cannot kill #0"),
                            internalKill);
}
 ...
 */
make_pagehandler(tm_priority, tm_priority_show, 0, 1, OS_PR_PRIORITY_SELECT,
                 pid, peekStack(0).param) {
 ...
 */
make_pagehandler(tm_priority_show, tm_priority_changeH, 0, 16,
                 OS_PR_PRIORITY_SHOW, pid, peekStack(1).param) {
  uint16_t const proc = peekStack(1).param;
 ...
 */
make_pagehandler(tm_priority_changeH, tm_priority_changeL, 0, 16,
                 OS_PR_PRIORITY, null, 0) {
 ...
 */
make_pagehandler(tm_priority_changeL, tm_priority_set, 0, 1, OS_PR_PRIORITY,
                 null, 0) {
 ...
 */
make_pagehandler(tm_priority_set, tm_null, 0, 0, OS_PR_PRIORITY, pid,
                 peekStack(4).param) {
  lcd_writeProgString(PSTR("Setting priority"));
  os_getProcessSlot(peekStack(4).param)->priority =
      ((peekStack(2).param & 0xF) << 4) + ((peekStack(1).param & 0xF));
  tm_done();
 ...
 *  If you implement this interface, make sure to conform to its parameter
 * specification \param index Strategy for which to lookup a name. Handling
 * undefined indices is mandatory. \param offsetOut Pointer to a location where
 * to store the length of the returned string. If NULL is passed, you are to
 * ignore the parameter. \returns A pointer to the prog string that contains the
 * name and is zero-terminated.
 */
typedef char const *StrategyNameLookup(uint16_t index,
                                       uint16_t *stringLengthOut);
 ...
 *    makeStrategyNameLookup(myLookupFunction, 4, PSTR("???"), {ST_ONE,
 * PSTR("one")}, {ST_TWO, PSTR("two")}) \param NAME The name of the function you
 * wish to create. \param STRINGLEN The length of the individual strings. \param
 * UNDEFSTRING The string to be returned if the strategy is unknown. \param
 * STRATEGIES Varying list of strategy options.
 */
  ...
  static char const *NAME(uint16_t ss, uint16_t *stringLengthOut) {            \
    struct {                                                                   \
      ...
      char const *name;                                                        \
        ...
        STRATEGIES{ss, UNDEFSTRING} /* <--- sentinel element (guarantees the   \
                                       loop terminates!) */                    \
    };                                                                         \
    if (stringLengthOut)                                                       \
 ...
 *  \returns true.
 */
static bool strategySelector(ParamStack const *p, StrategyNameLookup *names,
                             uint16_t curr) {
 ...
 *  \returns true.
 */
static bool strategyChanger(ParamStack const *p, StrategyNameLookup *names,
                            uint16_t curr, void (*set)(void *, uint16_t),
                            void *passThrough) {
...
makeStrategyNameLookup(
    getSchedulingStratNames, 0x18, 0,
    // 123456789abcdef0123456789ABCDEF0
     ...
     */
    make_pagehandler(tm_scheduling, tm_scheduling_set, 0, 1,
                     OS_PR_SCHEDULING_SELECT, ss, peekStack(0).param) {
  return strategySelector(p, getSchedulingStratNames,
                          os_getSchedulingStrategy());
}
 ...
 */
make_pagehandler(tm_scheduling_set, tm_null, 0, 0, OS_PR_SCHEDULING, ss,
                 peekStack(1).param) {
 ...
 */
make_pagehandler(tm_heap, tm_heap2, 0, 4, OS_PR_SHOW_HEAP, heapId,
                 peekStack(0).param) {
 ...
 *   - erase everything
 */
make_pagehandler(tm_heap2, tm_heap_strategy, 0, MS_MAX_COUNT,
                 OS_PR_ALWAYS_ALLOW, null, 0) {
  ...
  case 1: {
    ...
    result->param = 0;
    result->range = (os_getUseSize(heap) + TM_MAP_ENTRIES_PER_PAGE - 1) /
                    TM_MAP_ENTRIES_PER_PAGE;
    break;
  }
  case 2: {
...
makeStrategyNameLookup(getAllocationStratNames, 0x10, 0,
    ...
    /*!
     *  The page to select an allocation strategy for the previously selected
     * heap.
     */
    make_pagehandler(tm_heap_strategy, tm_heap_strategy_set, 0, 1,
                     OS_PR_ALLOCATION_SELECT, as, peekStack(0).param) {
  Heap *const heap = os_lookupHeap(peekStack(2).param);
  return strategySelector(p, getAllocationStratNames,
                          os_getAllocationStrategy(heap));
}
 ...
 */
make_pagehandler(tm_heap_strategy_set, tm_null, 0, 0, OS_PR_ALLOCATION, as,
                 peekStack(1).param) {
  Heap *const heap = os_lookupHeap(peekStack(3).param);
  return strategyChanger(p, getAllocationStratNames,
                         os_getAllocationStrategy(heap), setAS, heap);
}

static MemValue derefMap(Heap const *heap, MemAddr usePtr) {
  uint16_t const uOff = usePtr - os_getUseStart(heap);
  return (heap->driver->read(os_getMapStart(heap) + uOff / 2) >>
          (((~uOff) & 1) << 2)) &
         0xF;
}
...
make_pagehandler(tm_heap_contents, tm_null, 0, 0, OS_PR_SHOW_HEAP, null, 0) {
  Heap *const heap = os_lookupHeap(peekStack(2).param);
  uint16_t addr =
      os_getUseStart(heap) + peekStack(0).param * TM_MAP_ENTRIES_PER_PAGE;
  uint8_t i, j;
  for (i = 0; i < 2; i++) {
...
make_pagehandler(tm_heap_erase, tm_heap_erase2, 0, 1, OS_PR_ERASE_HEAP, heapId,
                 peekStack(2).param) {
...
make_pagehandler(tm_heap_erase2, tm_null, 0, 0, OS_PR_ERASE_HEAP, heapId,
                 peekStack(3).param) {
  ...
  for (ptr = start; ptr < end; ptr++) {
    ...
    uint8_t progress =
        (100ul * (uint16_t)(ptr - start)) / (uint16_t)(end - start);
    if (((uint16_t)progress * 32ul) / 100ul !=
        ((uint16_t)lastProgress * 32ul) / 100ul) {
