...
ISR(TIMER2_COMPA_vect) {
  ...
  // Laufzeitkontext auf dem Prozessstack des aktuellen Prozesses sichern //step
  // 2
  saveContext();
  ...
  // nach dem Setzen des Stackpointers auf den Scheduler-Stack die Prüfsumme des
  // Prozessstacks des unterbrochenen Prozesses ermittelt und abspeichert.
  ...
  } else if (os_processes[currentProc].state != OS_PS_UNUSED &&
  ...
  // BLOCKED prozesse sollen mindestens einmal aussetzen. Das haben sie nach dem
  // switch gemacht.
 ...
 *  registered) and the index is returned. On failure, INVALID_PROGRAM is
 * returned. Note, that this function is not used to register the idle program.
 *
...
ProgramID os_registerProgram(Program *program) {
  ...
  while (os_programs[slot] && os_programs[slot] != program &&
         slot < MAX_NUMBER_OF_PROGRAMS) {
    slot++;
  }
...
/*!
 *  Used to check whether a certain program ID is to be automatically executed
 * at system start.
 *
 ...
 * \return The pointer to the according function, or NULL if programID is
 * invalid.
 */
 ...
 * \return The id to the according slot, or INVALID_PROGRAM if program is
 * invalid.
 */
 ...
 *
 *  \param programID The program id of the program to start (index of
 * os_programs). \param priority A priority ranging 0..255 for the new process:
...
ProcessID os_exec(ProgramID programID, Priority priority) {
  ...
  // If freeIndex -1 then no free place in  array found and we can return
  // INVALID_PROCESS
  if (freeIndex == -1) {
  ...
  newProcess->state =
      OS_PS_READY; // here newProcess is a point, so has to use ->
  ...
  // move the higher 8 bits to the lower position, and the uint8_t then takes
  // the lower 8 bits
  bytesDesFunktionsregisters[1] = ptrFktZeiger & 0x00FF; // LOW  Bytes
  // uint8_t automatically abandon the higher 8 bits //can I just: uint8_t
  // low_byte = (uint16_t)currentProgramPointer?
  ...
  *(newProcess->sp.as_ptr) = bytesDesFunktionsregisters[0]; // HIGH Bytes
  ...
  // Da alle Register mit 0 inizialisiert werden sollen, k�nnen wir 33x 8-Bit
  // mit 0en f�llen
  for (uint8_t i = 0; i < 33; i++) {
    *(newProcess->sp.as_ptr) = 0b00000000; // all set as 0b00000000
    newProcess->sp.as_int--; // Dekrement stackpointer // alternative: set by
                             // PROCESS_STACK_BOTTOM(PID)
  }
 ...
 *  \return A pointer to the memory of the process at position pid in the
 * os_processes array.
 */
Process *os_getProcessSlot(ProcessID pid) { return os_processes + pid; }

/*!
 *  A simple getter for the slot of a specific program.
 *
 *  \param programID The ProgramID of the process to be handled
 *  \return A pointer to the function pointer of the program at position
 * programID in the os_programs array.
 */
 ...
 */
ProcessID os_getCurrentProc(void) { return currentProc; }
 ...
 */
SchedulingStrategy os_getSchedulingStrategy(void) { return schedulingStrategy; }
...
void os_leaveCriticalSection(void) {
  ...
  // check if critical section count is zero / if were no longer in the critical
  // section
  if (criticalSectionCount == 0) {
    // activate scheduler
    TIMSK2 |= 0b00000010;
 ...
 *
 *  \param pid The ID of the process for which the stack's checksum has to be
 * calculated. \return The checksum of the pid'th stack.
 */
...
bool os_kill(ProcessID pid) {
  ...
  while (pid == currentProc &&
         criticalSectionCount >
             1) {              // only if (pid==currentProc) we endlessloop
...
void os_yield() {
  ...
  SREG =
      GIEB | (SREG & 0b0111111); // SREG=0bYXXXXXXXX; GIEB=0bX00000000 right now
