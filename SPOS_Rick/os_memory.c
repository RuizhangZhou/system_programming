...
MemAddr getMapAddrForUseAddr(Heap const *heap, MemAddr addr) {
	...
	addr -= heap->startOfUseArea;
	// halbieren
	addr /= 2;
	// relative position im map-bereich
	return addr + heap->startOfMapArea;
}
...
void setProcVisitBit(Heap *heap, MemAddr addr) {
	...
	heap->visitedAreas[os_getCurrentProc() - 1] |= (1 << (16 * (addr - heap->startOfUseArea) / heap->sizeOfUseArea));
}
...
void assertAddrInUseArea(Heap const *heap, MemAddr addr) {
	if (addr < heap->startOfUseArea) {
		os_error("!!  expected  !!!!  use addr  !!");
	}
	if (addr >= (heap->startOfUseArea + heap->sizeOfUseArea)) {
		os_error("!!  expected  !!!!  use addr  !!");
	}
}
...
void os_freeOwnerRestricted(Heap *heap, MemAddr addr, ProcessID owner) {
	...
	while (addr < (heap->startOfUseArea + heap->sizeOfUseArea) && os_getMapEntry(heap, addr) == 0xF) {
		setMapEntry(heap, addr, 0);
		addr++;
	}
...
AllocStrategy os_getAllocationStrategy (Heap const *heap) {
	return heap->allocationStrategy;
}

//! Changes the memory management strategy.
void os_setAllocationStrategy (Heap *heap, AllocStrategy allocStrat) {
	os_enterCriticalSection();
	heap->allocationStrategy = allocStrat;
	os_leaveCriticalSection();
}
...
void os_freeProcessMemory (Heap *heap, ProcessID pid) {
	...
	for (int j = 0; j < 16; j++) {
		...
		if (heap->visitedAreas[pid - 1] & (1 << j)) {
			MemAddr offset = (heap->sizeOfUseArea * j) / 16;
			// + 1 als "ceiling" statt "floor", weil sonst durch Rundungsfehler die letzte Adresse des Bereichs übersehen werden könnte.
			// und der Sonderfall j == 15, da wir dann "nur" sizeOfUseArea und net + 1 haben wollen, da sonst os_getMapEntry "überlaufen" könnte
			MemAddr upfset = j + 1 != 16 ? (heap->sizeOfUseArea * (j+1)) / 16 + 1 : heap->sizeOfUseArea;
			// dann den bereich durchsuchen um zu free'en
			for (MemAddr i = heap->startOfUseArea + offset; i < (heap->startOfUseArea + upfset); i++) {
