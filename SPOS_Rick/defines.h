...
//! Maximum number of programs that can be known by the os (<17, 255 is
//! invalid).
...
#define STACK_SIZE_PROC                                                        \
  (((AVR_MEMORY_SRAM / 2) - STACK_SIZE_MAIN - STACK_SIZE_ISR) /                \
...
//! The bottom of the memory chunks for all process stacks. That is the highest
//! address.
#define BOTTOM_OF_PROCS_STACK (BOTTOM_OF_ISR_STACK - STACK_SIZE_ISR)

//! The bottom of the memory chunk with number PID.
#define PROCESS_STACK_BOTTOM(PID)                                              \
  (BOTTOM_OF_PROCS_STACK - ((PID)*STACK_SIZE_PROC))

//由于我们的Project 300多Byte,所以需要大于(100+300) 作为栈底地址
#define HEAPBOTTOM 0x500
//栈顶
#define HEAPCEILING PROCESS_STACK_BOTTOM(MAX_NUMBER_OF_PROCESSES)
