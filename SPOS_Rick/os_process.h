...
typedef enum { DONTSTART, AUTOSTART } OnStartDo;
 ...
 *  not even compile. Make sure to always pass natural numbers as first
 * argument. The second macro parameter specifies whether you want this program
 * to be auto-magically executed when the system boots. If you pass 'AUTOSTART',
 * it will create a process for this program while initializing the scheduler.
 * If you pass 'DONTSTART' instead, only the program will be registered (which
 * you may execute manually). Use this macro in this fashion:
 *
 *    PROGRAM(3, AUTOSTART) {
 *      foo();
 *      bar();
